...
Page({
  ...
  selectAuthorized(){
    wx.navigateTo({
      url: '../selectAuthorized/selectAuthorized'
    })
  },
  ...
  onLoad: function (options) {
    ...
    if(options.appId && '' != options.appId){
      ...
      wx.setStorageSync('currentAppId',options.appId);
      wx.setNavigationBarTitle({
        title:"邀请您加入"+options.projectName,
      }) 
    }
      ...
      if(res.data.code && 200 == res.data.code){
        if(res.data.data){
          ...
          that.setData({
            ...
            ['form.code']:res.data.data.jobNumber,
  ...
  send(data){
    const that = this;
    let appId = that.data.appId;
    req.request("post","person/invitation",{
      "appId": appId,
      "grantDeviceIdList": [],
